...
def compute_policy_loss(
    ...
    tau_positive: float,
    tau_negative: float,
    loss_type: Literal["default", "gspo", "gspo_token", "cispo", "sapo", "areal"],
    loss_avg_mode: Literal["token", "seq"],
    prox_log_probs: torch.Tensor | None = None, 
    **kwargs,
    ...
    elif loss_type == "areal":
        # prox_log_probs: 本次训练 step 开始前的模型 log probs（πprox）
        # old_log_probs:  生成数据时的行为策略 log probs（πbehav）
        assert prox_log_probs is not None, "areal loss_type requires prox_log_probs"
        
        # importance ratio w.r.t. proximal policy
        log_ratio_prox = log_probs - prox_log_probs          # log(πθ / πprox)
        ratio_prox = torch.exp(torch.clamp(log_ratio_prox, -10.0, 10.0))
        clipped_ratio_prox = torch.clamp(ratio_prox, 1.0 - clip_ratio_low, 1.0 + clip_ratio_high)
        
        # scaling factor: πprox / πbehav（常数，detach 掉）
        scale = torch.exp((prox_log_probs - old_log_probs).detach())
        
        pg_loss  = -advantages * ratio_prox * scale
        pg_loss2 = -advantages * clipped_ratio_prox * scale
        final_pg_loss = torch.max(pg_loss, pg_loss2)   # PPO clip
