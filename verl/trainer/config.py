...
class TrainerConfig:
    ...
    """[async mode] max allowed staleness of experience weight version relative to current training weight version.
    staleness_tolerance=2 means at training wv=N, accept experiences from wv >= N-2.
    Larger values = more throughput but potentially less on-policy. 0 = fully on-policy (defeats the purpose)."""
