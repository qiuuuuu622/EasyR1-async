...
class FSDPWorker(Worker):
    ...
    def _build_rollout(self) -> None:
        ...
        # ------------------------------------------------------------------ #
        # Server mode: vLLM runs in a separate process; we only need a client #
        # ------------------------------------------------------------------ #
        ...
        # ------------------------------------------------------------------ #
        # Default (SPMD) mode: vLLM colocated with training workers          #
        # ------------------------------------------------------------------ #
    ...
    def _build_rollout_cluster_mode(self, server_urls: list[str]) -> None:
        """
        Cluster mode: connect to multiple RolloutServers for distributed rollout.
 
        In this mode:
          - self.rollout is None (generation is done remotely)
          - self.rollout_client is a RolloutClusterClient managing N servers
          - self.rollout_sharding_manager wraps the cluster client
          - GPU memory on training workers is fully available for FSDP
 
        Supports rolling weight update, least-pending dispatch, and multi-source
        sample aggregation. See RolloutClusterClient for details.
        """
        from .rollout.cluster import RolloutClusterClient
        ...
        self.rollout = None  # no local rollout in cluster mode
        self.rollout_sharding_manager = FSDPVLLMServerShardingManager(
            rollout_client=self.rollout_client,
            use_param_offload=self._use_param_offload,
        )
        self.print_rank0(
            f"[Cluster Mode] RolloutClusterClient initialized with "
    ...
    def _build_rollout_server_mode(self, server_url: str) -> None:
        """
        Server mode: connect to a remote RolloutServer instead of running vLLM locally.

        In this mode:
          - self.rollout is None (generation is done remotely)
          - self.rollout_sharding_manager wraps a RolloutClient
          - GPU memory on training workers is fully available for FSDP
        """
        ...
        self.rollout = None  # no local rollout in server mode
        self.rollout_sharding_manager = FSDPVLLMServerShardingManager(
            rollout_client=self.rollout_client,
            use_param_offload=self._use_param_offload,
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            estimated_flops, promised_flops, mfu = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
    ...
    def submit_prompts(self, new_batch: DataProto, gen_batch: DataProto):
        """提交 (new_batch, gen_batch) 到 RolloutServer（非阻塞）。
        Server 负责 generate → repeat → union → 存入经验池。"""
        assert self._has_rollout
        if self.rollout is None:  # server mode
    ...
    def pull_samples(self, target_samples: int, min_weight_version: int = 0, timeout: float = 300.0):
        """从经验池拉取至少 target_samples 条 sample（阻塞等待）。
        返回 dict: {data: DataProto, weight_versions: list, total_samples: int}"""
        assert self._has_rollout
        if self.rollout is None:  # server mode
    ...
    def push_weights_only(self, flush_cache: bool = False):
        """仅推送权重到 server（用于 training step 后的权重同步）"""
        if self.rollout is None:  # server mode
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        # Server mode: delegate generation entirely to the remote RolloutServer
        if self.rollout is None:
            output = self.rollout_client.generate(prompts)
            output = output.to("cpu")
            return output

        # SPMD mode: local vLLM generation
    ...
    def compute_log_probs(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            output = DataProto.from_dict(
                tensors={"prox_log_probs": output}, meta_info={"temperature": self.config.rollout.temperature}
            )
